...
Begin VB.Form TOBR103 
   ...
   Begin VB.ComboBox cboImposto 
      ...
      ForeColor       =   &H00800000&
      Height          =   330
      ItemData        =   "TOBR103.frx":031A
      Left            =   720
      List            =   "TOBR103.frx":031C
      Sorted          =   -1  'True
   ...
   Begin Threed.SSFrame fraValor 
      ...
      Begin VTOcx.txtVISUAL txtValorITBI 
         ...
         AlinhamentoTexto=   1
      End
      Begin VTOcx.txtVISUAL txtValorFGTS 
         Height          =   330
         Left            =   345
         TabIndex        =   12
         Top             =   1280
         ...
         CorRotulo       =   192
      End
      Begin VTOcx.txtVISUAL txtAliquotaFGTS 
         Height          =   330
         Left            =   3205
         TabIndex        =   15
         Top             =   1275
         ...
         CorRotulo       =   192
      End
      Begin VTOcx.txtVISUAL txtSubTotalAliquotaFGTS 
         Height          =   330
         Left            =   6555
         TabIndex        =   55
         Top             =   1275
...
'                TotalImposto = (CDbl(Nvl(Trim(txtValorImovel), 0)) * (Aliquota / 100))
        ...
        Transf.GravarBCP CodPagamento, Date, txtIm, txtCgc, " " & txtCedente, " " & txtEnderecoCedente, txtImovel, " " & txtAdquirente, " " & txtEnderecoAdquirente, _
             CDbl(Nvl(Trim(txtValorAvista), 0)), CDbl(Nvl(Trim(txtValorFinanciado), 0)), txtValorImovel, BaseDeCalculo, _
            CDbl(TotalImposto), CboNatureza.Text, txtObservacao, Nvl(txtAliquotaPropria, 0), Nvl(txtAliquotaFinanciada, 0), Processo, Nvl(txtAliquotaFGTS, 0), Nvl(txtValorFGTS, 0)
        Bdados.FechaTabela Rs
...
Private Sub cmd_Click(Index As Integer)
    ...
    Select Case cmd(Index).Caption
        Case "&Emitir ITBI"
            If CDbl(Nvl(Trim(txtValorImovel), 0)) <> (CDbl(Nvl(Trim(txtValorFinanciado), 0)) + CDbl(Nvl(Trim(txtValorAvista), 0))) + CDbl(Nvl(Trim(txtValorFGTS), 0)) Then
...
Private Sub txtAliquotaFGTS_Change()
    Calcula
End Sub
...
Private Sub txtValorAvista_LostFocus()
    ...
    'txtValorFGTS = CDbl(Nvl(Trim(txtValorImovel), 0)) - CDbl(Nvl(Trim(txtValorFinanciado), 0)) - CDbl(Nvl(Trim(txtValorAvista), 0))
...
Private Sub Calcula()
    ...
    txtSubTotalAliquotaFinanciada = CDbl(Nvl(txtAliquotaFinanciada, 0)) * CDbl(Nvl(txtValorFinanciado, 0)) / 100
     ...
     txtSubTotalAliquotaFGTS = CDbl(Nvl(txtAliquotaFGTS, 0)) * CDbl(Nvl(txtValorFGTS, 0)) / 100
    
    'txtValorImovel = CDbl(Nvl(Trim(txtValorFinanciado), 0)) + CDbl(Nvl(Trim(txtValorAvista), 0))
    
    
    txtValorITBI = CDbl(Nvl(txtSubTotalaliquotaPropria, 0)) + CDbl(Nvl(txtSubTotalAliquotaFinanciada, 0)) + CDbl(Nvl(txtSubTotalAliquotaFGTS, 0))
