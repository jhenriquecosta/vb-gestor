...
Private Sub cmdSalvar_Click()
    ...
    NumAidf = Grafica.GeraNumero(AIDF)
    If Trim(txtIm) = Trim(txtImGrafica) Then
        If Confirma("Contribuinte não pode ser igual a gráfica." & vbCrLf & "Deseja continuar?") = False Then
    ...
    With NotaAidf
        ...
        .Especie = cboEspecie.Coluna(1).Valor
        .TipoAidf = cboEspecie.Coluna(1).Valor
        .Documento = cboDoc.Coluna(1).Valor
        If .GravarNota Then
            Dim Pos As Integer
            'Pos = InStr(cboDoc.Text, " - ")
            Util.Informa cboDoc.Text & " emitida com sucesso " & vbCrLf & "Nº gerado: " & NumAidf & "."
            If Util.Confirma("Deseja imprimir AIDF " & NumAidf & "" & "?") Then
                .Imprimir NumAidf
            End If
            Edita.LimpaCampos Me
...
Private Sub Form_Activate()
    ...
    End If
    cboDoc.PreencherGeral Bdados, "DOCUMENTACAO FISCAL"
    cboDoc.SetarLinha 1, 1
End Sub
...
Private Sub txtBlocos_Change()
    ...
    If cboEspecie.Coluna(1).Valor = 1 Then
