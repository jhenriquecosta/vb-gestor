...
Begin VB.Form PTBS501 
   ...
   Begin VB.Label Label2 
      BackColor       =   &H000000FF&
      Caption         =   " Gerência de Desenvolvimento Regional"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
Private Sub AtualizaG()
    Call Util.MontaGrid(Bdados, Grid, "SELECT TGR_COD_GERENCIA AS Código, TGR_NOME AS Nome FROM TAB_GERENCIA", 700, 6000)
End Sub
...
Sub Atualizar(Codigo As String)
    ...
    Campos = "TGR_COD_GERENCIA,TGR_NOME"
    Valores = Bdados.PreparaValor(txtPar, txtDes)
    
    Call Bdados.GravaDados("TAB_GERENCIA", Valores, Campos, _
    "TGR_COD_GERENCIA = " & txtPar)
...
Private Sub cmdGravar_Click()
    ...
    If txtPar.Enabled = False And Trim(txtDes) = "" Then
        If Util.Confirma("Deseja mesmo apagar " & txtPar & "?") Then
            If Bdados.DeletaDados("TAB_GERENCIA", "TGR_COD_GERENCIA=" & txtPar) Then
                Util.Informa "Gerência apagado."
            End If
        End If
    Else
