...
Private Sub AtualizaG()
    Call Util.MontaGrid(Bdados, Grid, "SELECT TMU_COD_MUNICIPIO AS Código, TMU_NOME AS Nome FROM TAB_MUNICIPIO", 900, 5800)
End Sub
...
Sub Atualizar(Codigo As String)
    ...
    Campos = "TMU_COD_MUNICIPIO,TMU_NOME"
    Valores = Bdados.PreparaValor(txtPar, txtDes)
    
    Call Bdados.GravaDados("TAB_MUNICIPIO", Valores, Campos, _
    "TMU_COD_MUNICIPIO = " & txtPar)
...
Private Sub cmdGravar_Click()
    ...
    If txtPar.Enabled = False And Trim(txtDes) = "" Then
        If Util.Confirma("Deseja mesmo apagar " & txtPar & "?") Then
            If Bdados.DeletaDados("TAB_MUNICIPIO", "TMU_COD_MUNICIPIO=" & txtPar) Then
                Util.Informa "Município apagado."
            End If
        End If
    Else
