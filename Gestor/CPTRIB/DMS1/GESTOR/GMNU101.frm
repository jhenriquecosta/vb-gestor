...
On Error GoTo Trata
    ...
    If Util.Confirma("Tem certeza que deseja efetuar logoff de " & Usuario & "?") Then
        ...
        Comando.Texto BdSeg, "sp_logoff", 4 '4=adCmdStoredProc
...
Private Sub cmdLogon_Click()
    ...
    Dim Comando As Object
'    Set Comando = CreateObject("VSClass.VSComando")
'    Comando.Texto BdSeg, "sp_logon", 4 '4=adCmdStoredProc
'    Comando.setarParametro "tlg_usuario_sistema", 201, 1, 20, Usuario  '201=adLongVarChar,1=adParamInput
'    Comando.Executa
'    Set Comando = Nothing
...
Private Sub CarregaBar(Bar As ProgressBar, User As String)
    ...
    If BdSeg.AbreTabela(SQL, RS) Then
        Bar.Max = RS(0) + 1
    Else
        Bar.Max = 1
    End If
...
On Error GoTo Trata
    SenhaInicial = Temp.PegaParametro(BdSeg, "SENHA INICIAL")
    Me.Caption = Temp.PegaParametro(BdSeg, "SISTEMA") & " - " & Temp.PegaParametro(BdSeg, "FANTASIA")
    lblSistema = Temp.PegaParametro(BdSeg, "SISTEMA")
    lblCliente = Temp.PegaParametro(BdSeg, "CLIENTE")
    lblResp = Temp.PegaParametro(BdSeg, "RESPONSAVEL")
...
Private Sub CarregaTree(lst As TreeView, User As String)
    ...
    Fig = "VISUAL"
    lst.Nodes.Add , tvwFirst, "0VSIS", Temp.PegaParametro(BdSeg, "SISTEMA"), Fig
    ...
    If BdSeg.AbreTabela(SQL, RS1) Then
        Do Until RS1.EOF
            Fig = "CLOSE"
            ...
            If BdSeg.AbreTabela(SQL, RS2) Then
                Do Until RS2.EOF
                    Fig = "CLOSE"
                    ...
                    " TFO_TMO_COD_MODULO " & BdSeg.Concatena & " TFO_COD_FORMULARIO IN (SELECT DISTINCT TAU_TMO_COD_MODULO " & BdSeg.Concatena & " TAU_TFO_COD_FORMULARIO FROM " & _
                    " TAB_ACESSO_USUARIO WHERE TAU_TUS_COD_USUARIO = '" & _
                     User & "' AND TAU_TMO_COD_MODULO = '" & RS2(0) & "')" & _
                     " ORDER BY TFO_NOME"

                    If BdSeg.AbreTabela(SQL, RS3) Then
                        Do Until RS3.EOF
                            Fig = RS2(0) & RS3(0)
                        ...
                        Loop
                    End If
                    BdSeg.FechaTabela RS3
                    
                    
                    RS2.MoveNext
                Loop
            End If
            BdSeg.FechaTabela RS2
            RS1.MoveNext
        Loop
    End If
    BdSeg.FechaTabela RS1
...
On Error GoTo Trata
    ...
    If Mid(treSis.SelectedItem.Key, 1, 1) = "3" Then
        ...
        If treSis.SelectedItem.Tag <> Sistema Then
            BdSis.FechaBanco
            Conecta Usuario, treSis.SelectedItem.Tag
        End If
...
Private Sub txtCodigo_LostFocus()
    ...
    txtNome = Seguranca.ExisteUsuario(BdSeg, txtCodigo)
    If txtNome <> "" Then
        Cont = 0
    Else
...
Private Sub txtNova_LostFocus()
    ...
    If txtNova <> "" Then
        If NovaValida(txtNova) Then
            ...
            cmdLogon.Enabled = False
        Else
            Util.Informa "Nova senha inválida. Deve possuir de " & Temp.PegaParametro(BdSeg, "SENHA TAMANHO MIN") & " a " & Temp.PegaParametro(BdSeg, "SENHA TAMANHO MAX") & " caracteres alfa-numéricos."
            txtNova = ""
            txtNova.SetFocus
        End If
    End If
...
Private Sub Travar()
    ...
    Call BdSeg.AtualizaDados("tab_usuario", "0'", "tus_ativo", "tus_cod_usuario= '" & txtCodigo & "'")
End Sub
...
Private Sub txtSenha_LostFocus()
    ...
    If Not SenhaValida(txtCodigo, txtSenha) Then
        ...
        If Cont = 3 Then
            If Temp.PegaParametro(BdSeg, "TRAVAR") = "SIM" Then Travar
...
Public Sub AlteraSenha(User As String, Nova As String)
    On Error GoTo Trata
    Dim Valor As String
    Valor = BdSeg.PreparaValor(Seguranca.Criptografa(Nova))
    BdSeg.AtualizaDados "TAB_USUARIO", Valor, "TUS_SENHA", _
...
Public Function NovaValida(Senha As String) As Boolean
    NovaValida = False
    Senha = Trim(Senha)
    If Len(Senha) >= CInt(Temp.PegaParametro(BdSeg, "SENHA TAMANHO MIN")) And Len(Senha) <= CInt(Temp.PegaParametro(BdSeg, "SENHA TAMANHO MAX")) Then
...
Public Function Descricao(Sistema As String) As String
    ...
    Select Case Tipo
        Case 0 'Nada
            Descricao = Temp.PegaParametro(BdSeg, "DESCRICAO")
        Case 1 'Sistema
        ...
        Case 3 'Formulário
            SQL = "SELECT TFO_DESCR FROM TAB_FORMULARIO WHERE TFO_TMO_COD_MODULO " & BdSeg.Concatena & " TFO_COD_FORMULARIO = '" & Mid(Sistema, 2) & "'"
    End Select
    If SQL <> "" Then
        If BdSeg.AbreTabela(SQL, RS) Then
            Descricao = "" & RS(0)
