...
Private Sub cmdBuscar_Click()
    ...
    BuscarDebitos
    'AtualizAtualizacaoMonetariaSelic
...
TrataErro:
    ...
    Resume
End Sub
'--- INICIO ATUALIZACAO MONETARIA SELIC
Private Sub AtualizAtualizacaoMonetariaSelic()
    Dim Sql As String, tipoPeriodo As String, obrigacao As String, valorOriginal As String, DataVencimento As Date, periodoAtual As String
    Dim i As Integer
    For i = 1 To lstParcelas.ListItems.Count
        obrigacao = lstParcelas.ListItems(i)
        DataVencimento = CDate(lstParcelas.ListItems(i).SubItems(4))
        valorOriginal = lstParcelas.ListItems(i).SubItems(5)
        AliquotaSelic obrigacao, valorOriginal, DataVencimento, tipoPeriodo
    Next i
    ExibirDados
End Sub
Private Sub AliquotaSelic(obrigacao As String, valorOriginal As String, DataVencimento As Date, tipoPeriodo As String)
    ...
    If Bdados.AbreTabela(Sql, rs) Then
        ...
        Sql = "update Tab_Conta_Contribuinte set tcc_correcao_monetaria=" & converteMonetario(CStr(Format(CDbl(Atualizacao), "##0.00"))) & " where tcc_codigo_conta=" & obrigacao
        Bdados.Executa (Sql)
        Sql = "update Tab_Conta_Contribuinte set tcc_saldo_atual=tcc_imposto_original + tcc_juros_atual + tcc_correcao_monetaria +tcc_multa_atual where tcc_codigo_conta=" & obrigacao
        Bdados.Executa (Sql)
...
Private Sub Form_Load()
    ...
    txtDtVence = Format(Now, "DD/MM/YYYY")
End Sub
