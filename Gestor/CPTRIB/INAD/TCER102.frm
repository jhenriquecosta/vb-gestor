...
Begin VB.Form TCER102 
   ...
   Begin ActiveTabs.SSActiveTabs tabCND 
      ...
      BeginProperty FontSelectedTab {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Tabs            =   "TCER102.frx":031A
      Images          =   "TCER102.frx":03CB
...
Private Sub BuscaCertidao()
    Certidao.BuscarCertidoes grdCPND, tcCPD, txtIm, txtImovel
End Sub
...
Dim Tipo As TipoCertidao
    Dim Obrig As New Obrigacao
    If txtIm = "" And txtImovel = "" Then ' Or txtIm <> "" And txtImovel <> "" Then
        Util.Avisa "Iforme Inscr.Municipal ou Cadastral."
        txtIm.SetFocus
        Exit Sub
    End If
...
'    End If
    ...
    Select Case TipoCertidao
        Case "TCER101": Tipo = tcCND
            ...
            If Obrig.CarregaListaObrigacao(grdDebitosVencido, txtIm, CStr(cboTributo.Coluna(0).Valor), , etlNaoPagos) Then
        ...
        Case "TCER102": Tipo = tcCPD
            Certidao.BuscarCertidoes grdCPND, Tipo, txtIm
              If Not Obrig.MostraObrigacaoGerada(grdDebitosVencido, CStr(cboTributo.Coluna(0).Valor), txtIm, _
                 , , , , , , , txtImovel, etlNaoPagos, IIf(Temp.PegaParametro(Bdados, "TRAZER SUBDIVIDA") = "SIM", True, False)) Then
                Util.Informa "Não é possível emitir a " & cabVISUAL1.Formulario & ". Não existem débitos."
                cmdEmitir.Enabled = False
                Exit Sub
              Else
                cmdEmitir.Enabled = True
...
Private Sub grdCPND_DblClick()
    'txtObs = grdCPND.ListItems(grdCPND.SelectedItem).SubItems(2)
End Sub
