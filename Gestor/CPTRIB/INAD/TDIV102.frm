...
Private Sub cmdExcluir_Click()
    ...
    If Confirma("Deseja excluir o item selecionado?") = True Then
        If Bdados.DeletaDados("TAB_PARAMETRO_DOC", condicao) Then
            ...
            If Bdados.DeletaDados("TAB_PARAMETRO_DAT", condicao) Then
                Util.Avisa "Operação concluída com sucesso."
                grdParametro.Preencher Bdados, "Select * from VIS_PARAMETRO order by 1,2,3"
   ...
   condicao = "TPD_CODIGO = '" & txtCodigo & cboDoc.Coluna(0).Valor & txtOrdem & "' and TPD_ANO = '" & txtExecicio & "' and TPD_IMPOSTO = '" & cboImposto.Coluna(0).Valor & "'"
    If Confirma("Deseja excluir o item selecionado?") = True Then
        If Bdados.DeletaDados("TAB_PARAMETRO_DAT", condicao) Then
            ...
            txtDescricao = ""
            txtExecicio.SetFocus
            GrdDados.Preencher Bdados, "select TPD_CODIGO as Código,tpd_ano as Ano,tpd_imposto as Imposto,tpd_descricao as Descrição   from TAB_PARAMETRO_DAT WHERE TPD_CODIGO = '" & txtCodigo & cboDoc.Coluna(0).Valor & txtOrdem & "'" & " order by 1,2"
        End If
    End If
End Sub
...
Private Sub cmdSalvar_Click()
    ...
    If txtExecicioFim = "" Then
        ...
        If Bdados.GravaDados("TAB_PARAMETRO_DAT", Valores, campos, condicao) Then
            Util.Avisa "Operação concluída com sucesso."
            txtExecicio = ""
            txtExecicioFim = ""
            cboImposto.ListIndex = -1
    ...
    ElseIf txtExecicioFim <> "" Then
        ...
        For Contador = 1 To i
            ...
            Bdados.GravaDados "TAB_PARAMETRO_DAT", Valores, campos, condicao
        Next
            Util.Avisa "Operação concluída com sucesso."
            txtExecicio = ""
            txtExecicioFim = ""
...
Private Sub CmdSalvarParametro_Click()
    ...
    condicao = "TPD_CODIGO = '" & txtCodigo & "' and TPD_TGL_CODIGO = '" & cboDoc.Coluna(0).Valor & "' and  TPD_ITEM = '" & txtOrdem & "'"
    If Bdados.GravaDados("tab_parametro_doc", Valores, campos, condicao) Then
        Util.Avisa "Operação concluída com sucesso."
        grdParametro.Preencher Bdados, "Select * from VIS_PARAMETRO order by 1,2,3"
        cmdLimparParametro_Click
    End If
...
Private Sub Form_Load()
    ...
    cboDoc.Preencher Bdados, "SELECT TGE_CODIGO,TGE_NOME FROM VIS_DOCUMENTOS_DAT ORDER BY TGE_CODIGO", 1
    grdParametro.Preencher Bdados, "Select Código,Documento,Item,Parametro,[Código Montado]  from VIS_PARAMETRO order by 1,2,3", 0, 1000, 800, 7000, 0
    CboParametro.Preencher Bdados, "SELECT  TPD_PARAMETRO FROM TAB_PARAMETRO_DOC"
...
Private Sub Pega_dados()
    ...
    SQL = "Select tpd_parametro from tab_PARAMETRO_doc where TPD_CODIGO = '" & txtCodigo & "' and TPD_CODIGO = '" & cboDoc.Coluna(0).Valor & "' and  TPD_ITEM = '" & txtOrdem & "'"
