...
Private Sub cmdDeletar_Click()
    If txtPar.Enabled = False And Trim(txtPar) <> "" Then
        If Util.Confirma("Deseja mesmo apagar " & txtPar & "?") Then
            If Bdados.DeletaDados("TAB_PARAM_DIV_ATI_NAO_TRIBU", "TPD_PARAMETRO='" & txtPar & "'") Then
...
Private Sub AtualizaG()
    Call Util.MontaGrid(Bdados, Grid, "SELECT TPD_PARAMETRO AS Parmâmetro, TPD_DESCRICAO AS Descrição FROM TAB_PARAM_DIV_ATI_NAO_TRIBU", 2500, 4300)
End Sub
...
Sub Atualizar(Codigo As String)
    ...
    Call Bdados.GravaDados("TAB_PARAM_DIV_ATI_NAO_TRIBU", Valores, Campos, _
