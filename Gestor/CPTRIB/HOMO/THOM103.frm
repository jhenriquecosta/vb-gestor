...
Private Sub cmdCancelar_Click()
    ...
    If grdAutos.ListItems.Count >= 1 Then
        If grdInfra.ListItems.Count >= 1 Then
            If Confirma("Deseja excluir?", "Ciap - Tributário") Then
                Bdados.AbreTrans
                If Bdados.AtualizaDados("tab_auto_infracao", StatusCancelado, "TAI_STATUS", "tai_cod_auto = '" & grdAutos.SelectedItem & "'") Then
                    If Bdados.DeletaDados("tab_infracao_auto", "tia_cod_auto = '" & grdAutos.SelectedItem & "'") Then
                        If Bdados.DeletaDados("TAB_OBRIGACAO_CONTRIBUINTE", "TOC_COD_OBRIGACAO = '" & grdAutos.SelectedItem & "'") Then
                            If Bdados.DeletaDados("tab_conta_contribuinte", "tcc_codigo_conta = '" & grdAutos.SelectedItem & "'") Then
                                Bdados.GravaTrans
                                Avisa "Operação concluída com sucesso."
                                cmdLimpar_Click
                            Else
                                Bdados.CancelaTrans
                                Avisa "Erro ao excluir Conta Contribuinte."
                            End If
                        Else
                            Bdados.CancelaTrans
                            Avisa "Erro ao Excluir obrigação."
                        End If
                        
                    Else
                        Bdados.CancelaTrans
                        Avisa "Erro ao exluir Infração."
                    End If
                Else
                    Bdados.CancelaTrans
                    Avisa "Erro ao Cancelar Auto de Infração."
                End If
            End If
        End If
    End If
