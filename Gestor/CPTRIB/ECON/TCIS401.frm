...
Private Sub cmdBuscar_Click()
    ...
    End If
    If Not Contribuinte.BuscarContribuintes(grdContribuinte, txtIm, txtCgc, txtRazao, txtFantasia, cboTipoLogr, cboLogr, txtBairro, txtNum, txtComplemento, _
                    txtCidade, CStr(cboAtivServ.Coluna(1).Valor), Iss, txtPeriodo1, txtPeriodo2, cboPonto, FiltroRpt, txtIAnterior, CStr(cboTipoCadastro.Coluna(1).Valor), CStr(cboSitCad.Coluna(1).Valor), txtvDataFormatura, CStr(CbovFuncionarioSUS.Coluna(1).Valor), CStr(cboPorte.Coluna(1).Valor)) Then
        Util.Avisa "Consulta sem resultados, UTILIZE OS DEMAIS FILTROS INDIVIDUALMENTE, como RAZAO / FANTASIA e ..."
    End If
    Exit Sub
trata:
...
Private Sub cmdImprimir_Click()
        If grdContribuinte.ListItems.Count > 0 Then
            ...
            If Left(cboModoImpressao, 1) = 0 Then
                If Not Rpt.DefinirArquivo(Bdados, App.Path & "\TFC_EMPRESA.rpt") Then Screen.MousePointer = 0: Exit Sub
                Rpt.Arvore = False
                ...
                Informa "Impressão concluída."
            ElseIf Left(cboModoImpressao, 1) = 1 Then
                If Not Rpt.DefinirArquivo(Bdados, App.Path & "\TCIS401LISTAGEM.rpt") Then Screen.MousePointer = 0: Exit Sub
                Rpt.Cabecalho Temp.PegaParametro(Bdados, "ESTADO"), Temp.PegaParametro(Bdados, "CLIENTE"), Temp.PegaParametro(Bdados, "SEMFAZ"), Temp.PegaParametro(Bdados, "SETOR")
                Rpt.Selecao = FiltroRpt
                Rpt.Arvore = False
                Rpt.Visualizar
                Screen.MousePointer = 0
            ElseIf Left(cboModoImpressao, 1) = 10 Then
                If Not Rpt.DefinirArquivo(Bdados, App.Path & "\TFICHA_CAD_ECONOMICO.rpt") Then Screen.MousePointer = 0: Exit Sub
                Rpt.Cabecalho Temp.PegaParametro(Bdados, "ESTADO"), Temp.PegaParametro(Bdados, "CLIENTE"), Temp.PegaParametro(Bdados, "SEMFAZ"), Temp.PegaParametro(Bdados, "SETOR")
'                Rpt.Rodape Temp.PegaParametro(Bdados, "RESPONSAVEL"), Temp.PegaParametro(Bdados, "CLIENTE"), Temp.PegaParametro(Bdados, "ENDERECO CLIENTE"), Aplicacoes.Usuario, Me.Name
            ...
            ElseIf Left(cboModoImpressao, 1) = 2 Then
                If Not Rpt.DefinirArquivo(Bdados, App.Path & "\TCIS403LISTAGEM.rpt") Then Screen.MousePointer = 0: Exit Sub
                Rpt.Cabecalho Temp.PegaParametro(Bdados, "ESTADO"), Temp.PegaParametro(Bdados, "CLIENTE"), Temp.PegaParametro(Bdados, "SEMFAZ"), Temp.PegaParametro(Bdados, "SETOR")
                Rpt.Selecao = FiltroRpt
                Rpt.Arvore = False
                Rpt.Visualizar
...
Private Sub Form_Load()
    ...
    cboModoImpressao.AddItem "0 - Um a um"
    cboModoImpressao.AddItem "1 - Listagem"
    cboModoImpressao.AddItem "2 - Atividade"
...
Private Sub grdContribuinte_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = 2 And grdContribuinte.ListItems.Count > 0 Then
        mnuCad.Caption = "Consultar Cadastro " & grdContribuinte.SelectedItem
                mnuLanca.Caption = "Consultar Lancamentos " & grdContribuinte.SelectedItem
